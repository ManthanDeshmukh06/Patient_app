...
export default function AppointmentStatusScreen({ route }) {
  const items = [
    { id: 1, title: 'Confirmed', time: 'Today, 10:30 AM', place: 'CityCare Hospital • OPD-2' },
    { id: 2, title: 'Pending Lab Results', time: 'Expected in 24 hrs', place: 'Central Lab' },
  ];
  ...
  return (
    <ScrollView style={styles.screen}>
      ...
      {route?.params?.doctor ? (
        <Card>
          <Text style={styles.kv}>Doctor: <Text style={styles.v}>{route.params.doctor}</Text></Text>
          <Text style={styles.kv}>Date: <Text style={styles.v}>{route.params.date} at {route.params.time}</Text></Text>
        </Card>
      ) : null}
