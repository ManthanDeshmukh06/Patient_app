...
const ForgotPasswordScreen = ({ navigation }) => {
  ...
  const sendOtp = async () => {
    ...
    try {
      setLoading(true);
      await axios.post(`${API_BASE_PHONE}/api/auth/send-otp`, { mobile });
  ...
  const resetPassword = async () => {
    ...
    try {
      setLoading(true);
      await axios.post(`${API_BASE_PHONE}/api/auth/reset-password`, {
        mobile,
        otp,
        newPassword,
      });
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 24, fontWeight: '700', textAlign: 'center', marginBottom: 30, color: '#333' },
