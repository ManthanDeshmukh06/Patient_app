...
export default function NewAppointmentModal({ route, navigation }) {
    ...
    const loadData = async () => {
      try {
        ...
        const resHospital = await axios.get(
          `http://10.0.2.2:3000/hospital/${storedHospitalId}`,
        ...
        const resDept = await axios.get(
          `http://10.0.2.2:3000/api/patient-appointments/departments/${storedHospitalId}`,
          { headers: { Authorization: `Bearer ${token}` } },
        );
    ...
    const fetchDoctors = async () => {
      ...
      try {
        ...
        const res = await axios.get(
          `http://10.0.2.2:3000/api/patient-appointments/doctors`,
          {
  ...
  useEffect(() => {
    const fetchSlots = async () => {
      ...
      try {
        ...
        const res = await axios.get(
          `http://10.0.2.2:3000/api/patient-appointments/available-slots`,
          {
  ...
  const saveAppointment = async () => {
    if (!selectedDoctor || !selectedDepartment || !time || !sessionType) return;
    ...
    try {
      ...
      await axios.post(
        `http://10.0.2.2:3000/api/patient-appointments/book`,
        payload,
        { headers: { Authorization: `Bearer ${token}` } },
      );
    ...
    >
      ...
      >
        <View style={styles.container}>
          ...
          >
            <MeetingIcon size={20} color={COLORS.subtext} />
            <Text
              style={[
                styles.input,
          ...
          <CustomButton
            ...
            disabled={
              !selectedDepartment || !selectedDoctor || !time || !sessionType
            }
          />
          <CustomButton
            title="Cancel"
...
const styles = StyleSheet.create({
  ...
  option: {
    padding: 15,
    borderBottomWidth: 1,
    borderBottomColor: '#eee', // softer separator
  },
  optionText: { fontSize: 16, color: '#000' }, // dark text
  ...
  timeText: { color: '#333' },
  timeActiveText: { color: '#fff', fontWeight: '700' },
